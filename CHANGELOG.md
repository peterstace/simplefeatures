...
- Add `NewEnvelopeXY` constructor, which builds an `Envelope` from variadic x
  ...
  `XY` values. It joins the other `XY` constructors such as `NewPointXY` and
  `NewLineStringXY`. The result is the smallest `Envelope` containing all of the
  coordinates, so no arguments gives the empty envelope and a single pair gives
  a point envelope. An odd number of arguments panics.
