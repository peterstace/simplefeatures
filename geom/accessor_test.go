...
func TestLineAccessor(t *testing.T) {
	...
	t.Run("start", func(t *testing.T) {
		got := line.StartPoint()
		want := Coordinates{XY: XY{1, 2}}
		expectCoordsEq(t, got, want)
	})
	t.Run("end", func(t *testing.T) {
		got := line.EndPoint()
		want := Coordinates{XY: XY{3, 4}}
		expectCoordsEq(t, got, want)
	})
...
func TestLineStringAccessor(t *testing.T) {
	...
	seq := ls.Coordinates()
	pt12 := Coordinates{XY: XY{1, 2}}
	pt34 := Coordinates{XY: XY{3, 4}}
	pt56 := Coordinates{XY: XY{5, 6}}

	t.Run("start", func(t *testing.T) {
		expectGeomEq(t, ls.StartPoint().AsGeometry(), NewPointC(pt12, XYOnly).AsGeometry())
	})
	t.Run("end", func(t *testing.T) {
		expectGeomEq(t, ls.EndPoint().AsGeometry(), NewPointC(pt56, XYOnly).AsGeometry())
	})
	t.Run("num points", func(t *testing.T) {
