...
func TestMaxDimension(t *testing.T) {
	for i, tc := range []struct {
		inputA, inputB, want imEntry
	}{
		{imEntryF, imEntryF, imEntryF},
		{imEntryF, imEntry0, imEntry0},
		{imEntryF, imEntry1, imEntry1},
		{imEntryF, imEntry2, imEntry2},
		{imEntry0, imEntry0, imEntry0},
		{imEntry0, imEntry1, imEntry1},
		{imEntry0, imEntry2, imEntry2},
		{imEntry1, imEntry1, imEntry1},
		{imEntry1, imEntry2, imEntry2},
		{imEntry2, imEntry2, imEntry2},
	} {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			for _, swap := range []struct {
				...
				t.Run(swap.name, func(t *testing.T) {
					...
					got := maxDimension(tc.inputA, tc.inputB)
					if got != tc.want {
						t.Errorf("%v %v want=%v got=%v",
							tc.inputA, tc.inputB, tc.want, got)
					}
				})
			}
		})
