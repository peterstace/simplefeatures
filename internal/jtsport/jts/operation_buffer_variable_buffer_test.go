...
const variableBuffer_DEFAULT_TOLERANCE = 1.0e-2

func TestVariableBuffer_ZeroWidth(t *testing.T) {
	checkVariableBuffer(t, "LINESTRING( 0 0, 6 6, 10 10)",
		0, 0,
		"POLYGON EMPTY")
}

func TestVariableBuffer_ZeroLength(t *testing.T) {
	checkVariableBuffer(t, "LINESTRING( 10 10, 10 10 )",
		0, 0,
		"POLYGON EMPTY")
}

func TestVariableBuffer_SegmentInverseDist(t *testing.T) {
	checkVariableBuffer(t, "LINESTRING (100 100, 200 100)",
		10, 1,
...
func TestVariableBuffer_SegmentSameDist(t *testing.T) {
	checkVariableBuffer(t, "LINESTRING (100 100, 200 100)",
		10, 10,
...
func TestVariableBuffer_OneSegment(t *testing.T) {
	checkVariableBuffer(t, "LINESTRING (100 100, 200 100)",
		10, 30,
...
func TestVariableBuffer_Segments2(t *testing.T) {
	checkVariableBuffer(t, "LINESTRING( 0 0, 40 40, 60 -20)",
		10, 20,
...
func TestVariableBuffer_LargeDistance(t *testing.T) {
	checkVariableBuffer(t, "LINESTRING( 0 0, 10 10)",
		1, 200,
...
func TestVariableBuffer_ZeroDistanceAtVertex(t *testing.T) {
	checkVariableBufferWithDistances(t, "LINESTRING( 10 10, 20 20, 30 30)",
		[]float64{5, 0, 5},
...
func TestVariableBuffer_ZeroDistancesForSegment(t *testing.T) {
	checkVariableBufferWithDistances(t, "LINESTRING( 10 10, 20 20, 30 30, 40 40)",
		[]float64{5, 0, 0, 5},
...
func TestVariableBuffer_Issue998_Spike(t *testing.T) {
	checkVariableBufferWithDistances(t, "LINESTRING (0.024520295 69.50077743, 0.000508719 74.50086084, 0 76.39546845)",
		[]float64{6.47, 6.9, 7},
...
func TestVariableBuffer_NoReverseSpike(t *testing.T) {
	checkVariableBufferWithDistances(t, "LINESTRING (0 70, 0 80)",
		[]float64{4, 7},
...
func TestVariableBuffer_NoShortCapSegments(t *testing.T) {
	checkVariableBufferWithDistances(t, "LINESTRING (6.85 78.25, 18 87)",
		[]float64{5, 9},
...
var variableBuffer_testFactory = Geom_NewGeometryFactoryDefault()

func checkVariableBuffer(t *testing.T, wkt string, startDist, endDist float64, wktExpected string) {
	t.Helper()
	geom := variableBufferRead(wkt)
	result := OperationBuffer_VariableBuffer_Buffer(geom, startDist, endDist)
	checkVariableBufferResult(t, result, wktExpected)
}

func checkVariableBufferWithDistances(t *testing.T, wkt string, dist []float64, wktExpected string) {
	t.Helper()
	geom := variableBufferRead(wkt)
	result := OperationBuffer_VariableBuffer_BufferWithDistances(geom, dist)
	checkVariableBufferResult(t, result, wktExpected)
}

func checkVariableBufferResult(t *testing.T, actual *Geom_Geometry, wktExpected string) {
	t.Helper()
	expected := variableBufferRead(wktExpected)
	variableBufferCheckEqual(t, expected, actual, variableBuffer_DEFAULT_TOLERANCE)
}

func variableBufferRead(wkt string) *Geom_Geometry {
	reader := Io_NewWKTReaderWithFactory(variableBuffer_testFactory)
...
func variableBufferCheckEqual(t *testing.T, expected, actual *Geom_Geometry, tolerance float64) {
	t.Helper()
